      ...
      try {
        if (user!.activeCompanyId) {
          if (!activeCompanyName) {
            const selected = await userSetActiveCompany(user!.id, user!.activeCompanyId);
            if (cancelled) {
              return;
            }
            if (selected.success) {
        ...
        const result = await userGetCompanies(user!.id);
        if (cancelled) {
          return;
        }
        ...
        if (result.companies.length === 1) {
          setSelecting(true);
          const selected = await userSetActiveCompany(user!.id, result.companies[0].id);
          if (cancelled) {
            return;
          }
