...
export default function StockTransferPage() {
  ...
  const [toProviderId, setToProviderId] = useState<number | null>(null);
  ...
  const selectedFromProvider = providers.find((provider) => provider.id === providerId) ?? null;
  const { dateLabel: drawDateLabel, dayLabel: drawDayLabel } = drawDateParts(selectedDraw);

  const toProviderOptions = useMemo(
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (providersResult.success) {
        ...
        setToProviderId((current) => {
          if (current != null) return current;
          const first = providersResult.providers[0]?.id ?? null;
          const second = providersResult.providers[1]?.id ?? null;
          return second ?? first;
        });
      }
  ...
  useEffect(() => {
    if (toProviderId === providerId) {
      const next = toProviderOptions.find((provider) => provider.id !== providerId);
      setToProviderId(next?.id ?? null);
    }
  }, [providerId, toProviderId, toProviderOptions]);
  ...
  const clearWorksheet = useCallback(() => {
    ...
    if (dirty) {
      setConfirm({ kind: 'clear', message: 'Clear all stock transfer data in this worksheet?' });
      return;
    }
    setRows([emptySaleRangeRow(defaultRate)]);
    setActiveRowIndex(0);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (providerId === toProviderId) {
      showToast('From and to providers must differ.', 'error');
      return;
    }
    ...
    if (ticketCount <= 0) {
      showToast('At least one ticket is required.', 'error');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        showToast(`${ticketCount} tickets transferred.`, 'success');
        setRows([emptySaleRangeRow(defaultRate)]);
        setActiveRowIndex(0);
        setVoucherNo('');
        setFieldsUnlocked(false);
  ...
  return (
    <>
      <LegacyTransactionShell
        ...
        centerTitle="Stock Transfer"
        contextLabel={
          selectedFromProvider
            ? `${selectedFromProvider.name} → ${
                providers.find((p) => p.id === toProviderId)?.name ?? '?'
              } (Sr ${activeRowIndex + 1})`
            : 'Stock Transfer'
        }
        accent="blue"
        ...
        alerts={
          <div className="flex shrink-0 items-center gap-3 border-b border-[#5e8ddd] bg-[#0a2566] px-4 py-2">
            <label className="flex min-w-[280px] items-center gap-2">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setToProviderId(value ? Number(value) : null);
                }}
                aria-label="To Provider"
              ...
              >
                ...
                {toProviderOptions.map((provider) => (
